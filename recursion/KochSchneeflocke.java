...
public class KochSchneeflocke 
{
    ...
    */
    ...
    {
    	// Rekursionsanker: wenn tiefe 0 erreicht zeichne linie  
      if (tiefe == 0) {
        StdDraw.line(x1, y1, x5, y5);
      } else {
        // berechnung der Punkte (x2, y2), (x3, y3), (x4, y4)
        // (5x - x1) / 3.0 und (y5 - y1) / 3.0 kommen mehrmals vor >> d.h wir müssen sie nur einmal berechnen
        double difX = (x5 - x1) / 3.0;
        double difY = (y5 - y1) / 3.0;
        
        double x2 = x1 + difX;
        double y2 = y1 + difY;

        double x3 = 0.5 * (x1 + x5) + Math.sqrt(3) * (y1 - y5) / 6.0;
        double y3 = 0.5 * (y1 + y5) + Math.sqrt(3) * (x5 - x1) / 6.0;

        // (2/3) * (x5 - x1) ist dasselbe wie 2.0 * difX 
        double x4 = x1 + 2.0 * difX;
        double y4 = y1 + 2.0 * difY;

        // Rekursionsschritt: rufe Function für 4 Teilstücke auf
